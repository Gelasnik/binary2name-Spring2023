...
class OutputConvertor:
    ...
    def __reduce_constraints(self, block_constraints: List[str]) -> List[str]:
        ...
        # TODO: OUR CODE
        paths_counter = min(self.sample_path, len(block_constraints))
        converted_block_constraints = []
        if paths_counter > 0:
            for path_num in self.path_priority_queue:
                ...
                for path_constraints in block_constraints:
                    if path_num == path_constraints[0]:
                        # TODO do we need to ignore an empty constraints?
                        if len(path_constraints[2]) == 0:
                            break
    ...
    def __convert_nodes(self, nodes: List) -> Dict:
        with open('conversion_config.json', 'r') as config_file:
            ...
            MAX_TOKENS_PER_CONSTRAINT = data['MAX_TOKENS_PER_CONSTRAINT']
        converted_nodes = {}
        # reduce constraints for all graph. Reduce number of paths that contribute constraint to nodes
        for node in nodes:
...
def main():
    ...
    parser.add_argument('--sample_constraint', type=int, default=3,
