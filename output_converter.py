...
class OutputConvertor:
    ...
    def choose_random_paths_to_remove(self, nodes: List):
        # previous version:
        lst = list(range(self.find_num_paths(nodes)))
        str_lst = [str(x) for x in lst]
        return random.sample(str_lst,
                             max(0, min(len(str_lst), math.ceil(len(str_lst) * (1 - self.sample_path)))))

    def count_paths_sizes(self, edges, root_key):
        import queue
        paths_size_dict = {}
        paths_start_points = [edge['dst'] for edge in edges if edge['src'] == root_key]
        for node_key in paths_start_points:
            visited = []
            q = queue.Queue()
            q.put(node_key)
            path_num = node_key.split("_")[-1]

            if path_num != "loopSeerDum" and path_num != "root":
                assert path_num not in paths_size_dict
                paths_size_dict[path_num] = 0
            ...
            while not q.empty():
                ...
                # Update path size dictionary if applicable
                if path_num != "loopSeerDum" and path_num != "root":
                    paths_size_dict[path_num] += 1

                # Find neighboring nodes
                neighboring_nodes = [edge['dst'] for edge in edges if edge['src'] == current_node]
                assert len(neighboring_nodes) < 2
                # Add unvisited neighboring nodes to the queue
                for neighbor_node in neighboring_nodes:
                    if neighbor_node not in visited:
                        q.put(neighbor_node)
        return paths_size_dict

    def choose_smallest_paths(self, nodes, edges):
        import heapq
        paths_size_dict = self.count_node_paths(nodes)

        return heapq.nsmallest(max(0, len(paths_size_dict) - self.sample_path), paths_size_dict,
                               key=paths_size_dict.get)

    def count_node_paths(self, nodes):
        paths_size_dict = {}
        for node in nodes:
            path_num = str(node["path_num"])
            if node["block_addr"] != "loopSeerDum" and path_num != -1:
                if path_num not in paths_size_dict:
                    paths_size_dict[path_num] = 0
                else:
                    paths_size_dict[path_num] += 1
        return paths_size_dict

    #     as a result only constraints from the longest paths are chosen

    def choose_biggest_paths(self, nodes, edges):
        import heapq
        paths_size_dict = self.count_node_paths(nodes)
        return heapq.nlargest(max(0, len(paths_size_dict) - self.sample_path), paths_size_dict, key=paths_size_dict.get)

    #     as a result only constraints from the smallest paths are chosen

    def create_paths_to_remove_list(self, nodes: List, edges):
        # self.paths_to_remove = self.choose_random_paths_to_remove(nodes) previous version
        res = self.choose_smallest_paths(nodes, edges)
        # print(f"-----------------\n {res} \n------------------")

        self.paths_to_remove = res
    ...
    def convert_json(self, filename: str):
        ...
        # try:
        self.create_paths_to_remove_list(initial_data['GNN_DATA']['nodes'], initial_data['GNN_DATA']['edges'])
        ...
        if self.nodes_total_num_constraints(converted_data['GNN_data']['nodes']) < 2:
            return False, None
...
def main():
    ...
    parser.add_argument('--sample_path', type=int, default=2,
