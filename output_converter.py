...
def is_mem(val: str) -> bool:
    """
    check if string 'mem' is in value
    """
    return 'mem' in val
...
def dissolve_function_call(str_call):
    # TODO why do we need find the function name and arguments here while we have json file where this
    #  information should be explicitly written
...
class ConstraintAst:
    ...
    def remove_filler_nodes(self, bad_name: str, argnum: int) -> None:
        """
        I am not so sure what is happening here, but I think we remove unwanted nodes with bad names to depth = argnum
        """
...
def are_constraints_similar(first: ConstraintAst, second: ConstraintAst) -> bool:
    """
    Check if two tree nodes are approximately similar. Checks children, values, etc.
    """
    ...
    if first.children == [] and second.children == []:
        ...
        if is_num(first.value) and is_num(second.value):
            ...
            if split_a[0] != split_b[0]:  # address
                return False
            # It is possible that addresses are similar but the program made changes so big that it is already not so
            # important that memory is in same place. It contains very different information
            if abs(split_a[1] - split_b[1]) > MEM_DIFF_THRESH:  # arbitrary id (incremented sequentially)
                return False
...
class OutputConvertor:
    def __init__(self, dataset_name: str, sample_path: int, sample_constraint: int):
        ...
        self.dest = CONVERTED_DS_PREFIX + dataset_name  # Currently: CONVERTED_DS_PREFIX = 'Converted_'
    ...
    def get_stats(self):
        """
        Statistic function analysis. It prints stats about constraints and nodes with constraints ratio.
        """
    ...
    def get_stats_json(self, filename):
        '''
        If successful, returns True and  dictionary with num_nodes_with_constraints/num_nodes ratio
        '''
        ...
        for node in nodes:
            ...
            # TODO Why we need to count the constraints?
    ...
    def __reduce_constraints(self, block_constraints: List[str]) -> List[str]:
        """
        Clears empty constraints and returns randomly selected constraints that are separated by '|'
        ...
        while i < len(constraint_asts):
            ...
            while j < len(constraint_asts) and not contradicting:
                if are_constraints_similar(constraint_asts[i], constraint_asts[j]):
                    if not duplicated:
                        # Only called here. When the nodes are similar.
                        # TODO why if nodes value are different it removes data, and keep only data type?
    ...
    def __convert_nodes(self, nodes: List) -> Dict:
        ...
        for node in nodes:
            ...
            if node['constraints'] != ['']:
                ...
                # Perform node-wide deduplication (remove nodes with similar constraints or contradicted)
